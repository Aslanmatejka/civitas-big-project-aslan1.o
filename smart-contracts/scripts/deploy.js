...
async function main() {
  ...
  // Write frontend .env.local (Vite loads this automatically and it overrides .env)
  const envContent =
    `# Auto-generated by deploy.js — do not edit manually\n` +
    `# Re-run: npm run contracts:deploy:local\n\n` +
    Object.entries(result.contracts)
      .map(([k, v]) => `VITE_${k.toUpperCase()}_ADDRESS=${v}`)
      .join("\n") +
    "\n";
  const webAppEnvPath = path.join(__dirname, "..", "..", "web-app", ".env.local");
  fs.writeFileSync(webAppEnvPath, envContent);
  console.log(`  › Contract addresses → web-app/.env.local`);
}
